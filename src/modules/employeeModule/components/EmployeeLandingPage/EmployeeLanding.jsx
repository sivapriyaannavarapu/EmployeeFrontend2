...
const getRolePrefix = () => {
  ...
  // 🔴 UPDATED LOGIC: Use Role Prefix in URL
  const handleSubmitEmployeeType = (employeeType, isSkip) => {
    const prefix = getRolePrefix(); // e.g. "do", "hr", "admin"
 
    if (employeeType === "Teach" && !isSkip) {
      // ✅ TEACH -> Skill Test URL: /scopes/employee/do-employee-onboarding/skilltest
      navigate(`/scopes/employee/${prefix}-employee-onboarding/skilltest`, {
        state: { employeeType }
      });
    } else {
      // ✅ NON-TEACH / SKIP -> New Onboarding URL: /scopes/employee/do-new-employee-onboarding/basic-info
 ...
 {/* <div className={styles.employeeGrid}>
  {Array.from({ length: 6 }).map((_, index) => (
    <div
      ...
      onClick={(e) => {
        e.stopPropagation(); // ⛔ prevents multiple navigations
